...
fscanf              (stdin,"String", _conditionals);
*/

_sampleFile     = PATH_TO_CURRENT_BF + "data/mcmc_samples";
...
_chainsToRun = 2;
_chainLength = 2000000;
_chainBurnin = 0.5*_chainLength;
_chainSamples = 100;
_concentration = 0.5;
...
if (!debug) {
    funcText = "";
    funcsToExport = {"0": "runMCMC", "1": "jointLogL", "2": "LogDrichletDensity", "3": "computeLogLFfromGridAndWeights", "4": "siteLikelihoodsGivenWeights", "5": "readMatrixFromText"};
    for (k = 0; k < Abs (funcsToExport); k+=1) {
        funcText += exportFunctionDefinition (funcsToExport[k]);
    }
      
    funcText += "\nfor (_chainIndex=start; _chainIndex<end; _chainIndex+=1) {runMCMC(_chainIndex,_gridInfo,_conditionals,_sampleFile,_chainLength,_chainBurnin,_chainSamples,_concentration);} return 0;";
    
    variablesToExport = "_gridInfo = \"" + _gridInfo + "\";\n" + 
                         "_conditionals = \"" + _conditionals + "\";\n" + 
    ...
    for (_r = _startPoint; _r < _chainsToRun; _r += 1){
        runMCMC(_r,_gridInfo,_sampleFile,_conditionals,_chainLength,_chainBurnin,_chainSamples,_concentration);
    }
...
function computeLogLFfromGridAndWeights (wts) {
    return +(((wts *(conditionals))["Log(_MATRIX_ELEMENT_VALUE_)"])+scalers);
}

//------------------------------------------------------------------------------------------------//

function siteLikelihoodsGivenWeights (wts) {
    return wts*(conditionals);
}

//------------------------------------------------------------------------------------------------//

function runMCMC (chainID,gridFile, sampleFile, conditionalFile, total,discard,expected_samples,_concentration_parameter){
    
    grid         = readMatrixFromText (gridFile);
    conditionals = readMatrixFromText (conditionalFile);
    scalers      = Transpose(conditionals [-1][Columns(conditionals)-1]);
    conditionals = Transpose (conditionals [{{0,0}}][{{Rows(conditionals)-1,Columns(conditionals)-2}}]);
    
    points             = Rows(grid);
    sites              = Columns(conditionals);
    normalize_by_site  = ({1,points}["1"])*(conditionals);
    normalized_weights = (conditionals)*({sites,sites}["1/normalize_by_site[_MATRIX_ELEMENT_ROW_]*(_MATRIX_ELEMENT_ROW_==_MATRIX_ELEMENT_COLUMN_)"]);
    ...
    for (k = 0; k < points; k+=1) {
        individualGridPointContribs [k] = (conditionals)[k][-1];
    }    
